...
func WaitForSecretInNamespaces(client *rancher.Client, clusterID, secretName string, namespaceList []*corev1.Namespace, shouldExist bool) error {
	return kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveSecondTimeout, defaults.TwoMinuteTimeout, true, func(ctx context.Context) (done bool, err error) {
		for _, ns := range namespaceList {
