...
// CreateSCIMUser builds a SCIM user with a random username and the given test-specific
// options, then delegates to shepherd's CreateUserWithTemplate which registers a session
...
func CreateSCIMUser(scimClient *scimclient.Client, ts *session.Session, externalID string, active bool) (string, string, error) {
	...
	resp, err := extauthscim.CreateUserWithTemplate(scimClient, ts, user)
	if err != nil {
		return "", "", err
	}
...
// CreateSCIMGroup builds a SCIM group with a random displayName and the given test-specific
// options, then delegates to shepherd's CreateGroupWithTemplate which registers a session
// cleanup that deletes the group when the session ends.
func CreateSCIMGroup(scimClient *scimclient.Client, ts *session.Session, externalID string) (string, string, error) {
	...
	resp, err := extauthscim.CreateGroupWithTemplate(scimClient, ts, group)
	if err != nil {
		return "", "", err
	}
...
func ObserveSCIMThrottle(scimClient *scimclient.Client, maxAttempts int) (bool, error) {
	resp, err := FirstThrottledResponse(scimClient, maxAttempts)
	if err != nil {
		return false, err
	}
	return resp != nil, nil
