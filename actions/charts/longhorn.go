...
func InstallLonghornChart(client *rancher.Client, payload PayloadOpts, values map[string]interface{}) error {
	...
	client.Session.RegisterCleanupFunc(func() error {
		chart, err := shepherdCharts.GetChartStatus(client, payload.Cluster.ID, payload.Namespace, longhornChartName)
		if err != nil || !chart.IsAlreadyInstalled {
			return err
		} else {
...
// This function has the purpose of helping to test RBAC with Longhorn.
func CreateAndDeleteLonghornVolume(client *rancher.Client, clusterID string, namespace string, storageClass storagev1.StorageClass) error {
