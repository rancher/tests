...
func GetExtTokenDefaultTTLMinutes(client *rancher.Client) (string, error) {
	extTokenSetting, err := settings.GetRancherSetting(client, "auth-token-max-ttl-minutes")
	if err != nil {
		return "", fmt.Errorf("error fetching auth-token-max-ttl-minutes: %w", err)
	}

	return extTokenSetting, nil
}
