...
// GetPasswordMinLength fetches the Rancher setting "password-min-length"
func GetPasswordMinLength(client *rancher.Client) (string, error) {
	passwordMinLength, err := settings.GetRancherSetting(client, "password-min-length")
	if err != nil {
		return "", fmt.Errorf("error fetching password-min-length: %w", err)
	}
...
// PasswordChangeRequest updates the password for a given user using imperative API
...
// CreateSelfUserRequest retrieves user ID by creating a SelfUser resource
...
// CreateGroupMembershipRefreshRequest triggers a group membership refresh for a user
