...
// Deelete
func verifyBindingsTemp(client *rancher.Client, clusterID string, userName string, roleTemplateName string, roleTemplateBindingName string, namespaces []string, expectedRoleBindingCount, expectedClusterRoleBindingCount int) error {
	ctx, err := clusterapi.GetClusterWranglerContext(client, clusterID)
	if err != nil {
		return err
	}
		...
		if expectedRoleBindingCount > 0 {
			...
			if clusterID == rbacapi.LocalCluster {
				if strings.Contains(roleTemplateBindingName, "prtb") {
					expectedRoleNames = append(expectedRoleNames, roleTemplateName+ProjectMgmtResourceAggregator)
				} else {
					expectedRoleNames = append(expectedRoleNames, roleTemplateName+ClusterMgmtResourceAggregator)
					if expectedRoleBindingCount > 1 {
						expectedRoleNames = append(expectedRoleNames, roleTemplateName+ProjectMgmtResourceAggregator)
					}
				}
			} else {
				expectedRoleNames = append(expectedRoleNames, roleTemplateName+ResourceAggregator)
			}

			for _, rb := range filteredRBs {
				found := false
				...
				if !found {
					return fmt.Errorf("unexpected RoleBinding RoleRef.Name %s, expected one of %v", rb.RoleRef.Name, expectedRoleNames)
				}
			}
		}
	}
	...
	if expectedClusterRoleBindingCount > 0 {
		...
		if clusterID == rbacapi.LocalCluster {
			if strings.Contains(roleTemplateBindingName, "prtb") {
				...
				expectedRoleNames = append(expectedRoleNames, roleTemplateName+ClusterMgmtResourceAggregator)
				if expectedClusterRoleBindingCount > 1 {
					expectedRoleNames = append(expectedRoleNames, roleTemplateName+ProjectMgmtResourceAggregator)
				}
			}
		} else {
		...
		for _, crb := range filteredCRBs {
			...
			if !found {
				return fmt.Errorf("unexpected ClusterRoleBinding RoleRef.Name %s, expected one of %v", crb.RoleRef.Name, expectedRoleNames)
			}
		}
	}
