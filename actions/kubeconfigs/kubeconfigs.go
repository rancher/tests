...
// GetKubeconfig retrieves a kubeconfig by name using public API
...
// UpdateKubeconfig updates an existing kubeconfig by name
func UpdateKubeconfig(client *rancher.Client, name string, updatedSpec extapi.KubeconfigSpec) (*extapi.Kubeconfig, error) {
	existing, err := client.WranglerContext.Ext.Kubeconfig().Get(name, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to get existing kubeconfig %s: %w", name, err)
	}

	existing.Spec = updatedSpec

	updated, err := client.WranglerContext.Ext.Kubeconfig().Update(existing)
	if err != nil {
		return nil, fmt.Errorf("failed to update kubeconfig %s: %w", name, err)
	}
...
func CreateDownstreamACEEnabledCluster(client *rancher.Client) (*v1.SteveAPIObject, *clusters.ClusterConfig, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create ACE enabled RKE2 cluster: %w", err)
	}

	return clusterObject, clusterConfig, nil
}

// GetClusterNameToID maps cluster names to their IDs from expectedClusterIDs.
func GetClusterNameToID(client *rancher.Client, expectedClusterIDs []string) (map[string]string, string, error) {
...
// DeleteClusterAndWait deletes the given cluster and blocks until it is fully removed.
func DeleteClusterAndWait(client *rancher.Client, cluster *management.Cluster) error {
	if cluster == nil {
		return nil
	}

	if err := client.Management.Cluster.Delete(cluster); err != nil {
		return fmt.Errorf("failed to initiate deletion of cluster %s: %w", cluster.ID, err)
	}

	err := kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveSecondTimeout, defaults.FifteenMinuteTimeout, true, func(ctx context.Context) (bool, error) {
		_, err := client.Management.Cluster.ByID(cluster.ID)
		if err != nil {
			if strings.Contains(err.Error(), "not found") {
				log.Infof("Cluster %s successfully deleted", cluster.Name)
				return true, nil
			}
			return false, nil
		}
	...
	if err != nil {
		return fmt.Errorf("cluster %s deletion polling failed: %w", cluster.Name, err)
	}
