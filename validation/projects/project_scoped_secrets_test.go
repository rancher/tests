...
func (pss *ProjectScopedSecretTestSuite) TestViewAllProjectsCannotAccessSecrets() {
	subSession := pss.session.NewSession()
	defer subSession.Cleanup()
...
func (pss *ProjectScopedSecretTestSuite) TestProjectScopedSecretWithRancherRestart() {
	subSession := pss.session.NewSession()
	defer subSession.Cleanup()
	...
	require.NoError(pss.T(), err, "Failed to add environment variable")
...
func (pss *ProjectScopedSecretTestSuite) TestNamespaceSecretRecreationAfterDeletion() {
	subSession := pss.session.NewSession()
	defer subSession.Cleanup()
	...
	targetNamespace := namespaceList[0]
	log.Infof("Deleting the propagated secret %s from namespace %s", createdProjectScopedSecret.Name, targetNamespace.Name)
	err := secrets.DeleteSecret(pss.client, pss.cluster.ID, targetNamespace.Name, createdProjectScopedSecret.Name)
	require.NoError(pss.T(), err, "Failed to delete secret from namespace")

	log.Info("Restart Rancher")
	err = deployment.RestartDeployment(pss.client, rbac.LocalCluster, rbac.RancherDeploymentNamespace, rbac.RancherDeploymentName)
	require.NoError(pss.T(), err, "Failed to add environment variable")

	log.Info("Verify that the deleted secret gets recreated in the namespace")
	err = secrets.WaitForSecretInNamespaces(pss.client, pss.cluster.ID, createdProjectScopedSecret.Name, []*corev1.Namespace{targetNamespace}, true)
	require.NoError(pss.T(), err, "Secret was not recreated in namespace")

	log.Info("Verify the secret in all the namespaces in the project.")
	err = secrets.ValidatePropagatedNamespaceSecrets(pss.client, pss.cluster.ID, createdProject.Name, createdProjectScopedSecret, namespaceList)
	require.NoError(pss.T(), err)

}

func (pss *ProjectScopedSecretTestSuite) TestVerifyNamespaceSecretDataReversion() {
	subSession := pss.session.NewSession()
	defer subSession.Cleanup()

	log.Info("Add environment variable CATTLE_RESYNC_DEFAULT and set it to 5 minutes")
	err := deployment.UpdateOrRemoveEnvVarForDeployment(pss.client, rbac.RancherDeploymentNamespace, rbac.RancherDeploymentName, rbac.CattleResyncEnvVarName, "5")
	require.NoError(pss.T(), err, "Failed to add environment variable")

	createdProject, namespaceList, createdProjectScopedSecret := pss.testProjectScopedSecret(pss.cluster.ID, corev1.SecretTypeOpaque, opaqueSecretData)

	log.Info("Update the secret data to something else in one of the namespaces in the project.")
	targetNamespace := namespaceList[0]
	nsSecret, err := secretsapi.GetSecretByName(pss.client, pss.cluster.ID, targetNamespace.Name, createdProjectScopedSecret.Name, metav1.GetOptions{})
	require.NoError(pss.T(), err)

	newData := map[string][]byte{
		"foo": []byte("YmFy"),
	}
	_, err = secrets.UpdateSecretData(pss.client, pss.cluster.ID, targetNamespace.Name, nsSecret.Name, newData)
	require.NoError(pss.T(), err)

	log.Info("Verify that the secret data in all the namespaces matches the original project-scoped secret.")
	err = kwait.PollUntilContextTimeout(context.TODO(), defaults.FiveMinuteTimeout, defaults.TenMinuteTimeout, false, func(ctx context.Context) (bool, error) {
		err = secrets.ValidatePropagatedNamespaceSecrets(pss.client, pss.cluster.ID, createdProject.Name, createdProjectScopedSecret, namespaceList)
		if err != nil {
			return false, err
		}
	...
	err = deployment.UpdateOrRemoveEnvVarForDeployment(pss.client, rbac.RancherDeploymentNamespace, rbac.RancherDeploymentName, rbac.CattleResyncEnvVarName, "")
	require.NoError(pss.T(), err, "Failed to remove environment variable")
}
