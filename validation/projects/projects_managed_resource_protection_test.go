...
func (p *ProjectsManagedResourceProtectionTestSuite) TestUserHasFullAccessToUnmanagedLimitRange() {
	...
	updatedMemory := updated.Spec.Limits[0].Default[corev1.ResourceMemory]
	expectedMemory := resource.MustParse("4Gi")
	require.Equal(p.T(), 0, expectedMemory.Cmp(updatedMemory), "Expected the unmanaged LimitRange default memory to be updated to 4Gi")
...
func (p *ProjectsManagedResourceProtectionTestSuite) TestNamespaceDeletionRemovesManagedResources() {
	...
	listOptions := metav1.ListOptions{
		LabelSelector: namespaceapi.ManagedResourceQuotaLabel + "=true",
		FieldSelector: "metadata.namespace=" + namespace,
	}
	rqList, err := adminContext.Core.ResourceQuota().List(corev1.NamespaceAll, listOptions)
	require.NoError(p.T(), err, "Failed to list Rancher-managed ResourceQuotas for the deleted namespace")
	require.Empty(p.T(), rqList.Items, "No Rancher-managed ResourceQuota should remain for the deleted namespace")

	lrList, err := adminContext.Core.LimitRange().List(corev1.NamespaceAll, listOptions)
	require.NoError(p.T(), err, "Failed to list Rancher-managed LimitRanges for the deleted namespace")
	require.Empty(p.T(), lrList.Items, "No Rancher-managed LimitRange should remain for the deleted namespace")
}
