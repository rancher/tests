...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbInheritanceWithProjectMgmtResources() {
	...
	createdProject, createdNamespaces, createdUser, _, _, _, err := acrCreateTestResources(acrp.client, acrp.cluster)
	require.NoError(acrp.T(), err)

	log.Info("Creating project role templates with project management plane resources.")
	childRules := policyRules["readPrtbs"]
	mainRules := policyRules["updatePrtbs"]
	...
	localCRs, err := getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, childRTName, mainRTName)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 8, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err := getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, childRTName, mainRTName)
	require.NoError(acrp.T(), err)
	...
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName, []string{childRTName})
	...
	err = verifyBindingsForPrtb(acrp.client, rbac.LocalCluster, &prtbs[0], nil, 0, 1)
	require.NoError(acrp.T(), err)

	log.Info("Verifying role bindings and cluster role bindings created for the user in the downstream cluster.")
	err = verifyBindingsForPrtb(acrp.client, acrp.cluster.ID, &prtbs[0], createdNamespaces, 1, 0)
	require.NoError(acrp.T(), err)
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbInheritanceWithRegularResources() {
	...
	createdProject, createdNamespaces, createdUser, createdDeployment, podNames, _, err := acrCreateTestResources(acrp.client, acrp.cluster)
	require.NoError(acrp.T(), err)
	...
	log.Info("Verifying role bindings and cluster role bindings created for the user in the local cluster.")
	err = verifyBindingsForPrtb(acrp.client, rbac.LocalCluster, &prtbs[0], nil, 0, 0)
	require.NoError(acrp.T(), err)
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbInheritanceWithMgmtAndRegularResources() {
	...
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, childRTName, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR")
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbInheritanceWithMultipleRules() {
	...
	createdProject, createdNamespaces, createdUser, createdDeployment, podNames, createdSecret, err := acrCreateTestResources(acrp.client, acrp.cluster)
	require.NoError(acrp.T(), err)
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbWithNoInheritance() {
	...
	mainRules := policyRules["readPrtbs"]
	...
	log.Info("Verifying the cluster roles created in the local cluster.")
	localCRs, err := getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, mainRTName)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 4, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err := getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, mainRTName)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 2, len(downstreamCRs.Items))

	log.Info("Verifying Aggregated Cluster Role (ACR) for mainRole in both the local and downstream clusters includes all the rules from both childRole and mainRole.")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName, nil)
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR")
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbInheritanceWithTwoPrtbs() {
	...
	mainRules1 := policyRules["updatePrtbs"]
	mainRules2 := policyRules["readPrtbs"]
	...
	log.Info("Verifying the cluster roles created in the local cluster.")
	localCRs, err := getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, childRTName1, mainRTName1, childRTName2, mainRTName2)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 12, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err := getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, childRTName1, mainRTName1, childRTName2, mainRTName2)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 8, len(downstreamCRs.Items))

	log.Info("Verifying Aggregated Cluster Role (ACR) for mainRole in both the local and downstream clusters includes all the rules from both childRole and mainRole.")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName1, []string{childRTName1})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName2, []string{childRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName1, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName2, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName1, []string{childRTName1})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName2, []string{childRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR")
	...
	log.Info("Verifying ProjectRoleTemplateBindings are created for the user")
	...
	err = verifyBindingsForPrtb(acrp.client, rbac.LocalCluster, &prtbs[0], nil, 0, 1)
	require.NoError(acrp.T(), err)
	err = verifyBindingsForPrtb(acrp.client, rbac.LocalCluster, &prtbs[1], nil, 0, 1)
	require.NoError(acrp.T(), err)

	log.Info("Verifying role bindings and cluster role bindings created for the user in the downstream cluster.")
	// Product Bug: https://github.com/rancher/rancher/issues/49265
	// err = verifyBindingsForPrtb(acrp.client, acrp.cluster.ID, &prtbs[0], createdNamespaces, 1, 0)
	// require.NoError(acrp.T(), err)
	// err = verifyBindingsForPrtb(acrp.client, acrp.cluster.ID, &prtbs[1], createdNamespaces, 1, 0)
	// require.NoError(acrp.T(), err)
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbNestedInheritance() {
	...
	childRules3 := policyRules["readPrtbs"]
	...
	log.Info("Verifying the cluster roles created in the local cluster.")
	localCRs, err := getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, childRTName1, childRTName2, childRTName3, mainRTName1)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 12, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err := getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, childRTName1, childRTName2, childRTName3, mainRTName1)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 8, len(downstreamCRs.Items))

	log.Info("Verifying ACRs in the local cluster.")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, childRTName3, []string{childRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName1, []string{childRTName1, childRTName2, childRTName3})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for main role")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, childRTName3, []string{childRTName1, childRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for child role 3")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, childRTName2, []string{childRTName1})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for child role 2")

	log.Info("Verifying ACRs in the downstream cluster.")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName1, []string{childRTName1, childRTName2, childRTName3})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR for main role")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, childRTName3, []string{childRTName1, childRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR for child role 3")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, childRTName2, []string{childRTName1})
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbMultipleLevelsOfInheritance() {
	...
	log.Info("Verifying the cluster roles created in the local cluster.")
	localCRs, err := getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, childRTName11, childRTName12, parentRTName1, childRTName21, parentRTName2, mainRTName1)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 18, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err := getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, childRTName11, childRTName12, parentRTName1, childRTName21, parentRTName2, mainRTName1)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 12, len(downstreamCRs.Items))

	log.Info("Verifying ACRs in the local cluster.")
	// Product Bug: https://github.com/rancher/rancher/issues/GH#49231
	// err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName1, []string{parentRTName2, childRTName12})
	// require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, mainRTName1, []string{childRTName11, childRTName12, parentRTName1, childRTName21, parentRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for main role")

	log.Info("Verifying ACRs in the downstream cluster.")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, mainRTName1, []string{childRTName11, childRTName12, parentRTName1, childRTName21, parentRTName2})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR for main role")
	...
	// require.NoError(acrp.T(), rbac.VerifyUserPermission(acrp.client, acrp.cluster.ID, createdUser, "list", "namespaces", "", "", true, false))
	require.NoError(acrp.T(), rbac.VerifyUserPermission(acrp.client, acrp.cluster.ID, createdUser, "get", "secrets", namespaceName, createdSecret[0].Name, true, false))
	require.NoError(acrp.T(), rbac.VerifyUserPermission(acrp.client, acrp.cluster.ID, createdUser, "list", "secrets", namespaceName, "", true, false))
	require.NoError(acrp.T(), rbac.VerifyUserPermission(acrp.client, acrp.cluster.ID, createdUser, "get", "projectroletemplatebindings", createdPrtb.Namespace, createdPrtb.Name, true, true))
	require.NoError(acrp.T(), rbac.VerifyUserPermission(acrp.client, acrp.cluster.ID, createdUser, "list", "projectroletemplatebindings", createdPrtb.Namespace, "", true, true))
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbUpdateRoleTemplateToAddInheritance() {
	...
	createdProject, createdNamespaces, createdUser, _, podNames, _, err := acrCreateTestResources(acrp.client, acrp.cluster)
	require.NoError(acrp.T(), err)
	...
	log.Info("Verifying the cluster roles created in the local cluster.")
	localCRs, err = getClusterRolesForRoleTemplates(acrp.client, rbacapi.LocalCluster, childRTName)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 2, len(localCRs.Items))

	log.Info("Verifying the cluster roles created in the downstream cluster.")
	downstreamCRs, err = getClusterRolesForRoleTemplates(acrp.client, acrp.cluster.ID, childRTName)
	require.NoError(acrp.T(), err)
	require.Equal(acrp.T(), 2, len(downstreamCRs.Items))
	...
	log.Info("Verifying Aggregated Cluster Role (ACR) for mainRole in both the local and downstream clusters includes all the rules.")
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, updatedMainRT.Name, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch local ACR")
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, mainRTName, nil)
	require.NoError(acrp.T(), err, "Failed to fetch local ACR for project-mgmt resources")
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, updatedMainRT.Name, []string{childRTName})
	require.NoError(acrp.T(), err, "Failed to fetch downstream ACR")
	...
	// 2 rolebindings in a namespace - check
	// err = verifyBindingsForPrtb(acrp.client, acrp.cluster.ID, &prtbs[0], createdNamespaces, 1, 0)
	// require.NoError(acrp.T(), err)
...
func (acrp *AggregatedClusterRolesPrtbTestSuite) TestPrtbUpdateRoleTemplateToRemoveInheritance() {
	...
	err = verifyProjectMgmtACR(acrp.client, rbacapi.LocalCluster, updatedMainRT.Name, []string{})
	require.NoError(acrp.T(), err)
	err = verifyMainACRContainsAllRules(acrp.client, rbacapi.LocalCluster, updatedMainRT.Name, []string{})
	require.NoError(acrp.T(), err)
	err = verifyMainACRContainsAllRules(acrp.client, acrp.cluster.ID, updatedMainRT.Name, []string{})
	require.NoError(acrp.T(), err)
	...
	log.Info("Verifying role bindings and cluster role bindings for the user in the downstream cluster.")
	// rolebinding mismatch - 2 instead of 1  - check
	// err = verifyBindingsForPrtb(acrp.client, acrp.cluster.ID, &prtbs[0], createdNamespaces, 1, 0)
	// require.NoError(acrp.T(), err)
