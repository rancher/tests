...
func (acrc *AggregatedClusterRolesCrtbTestSuite) TestCrtbInheritanceWithClusterMgmtResources() {
	...
	mainRules := rbac.PolicyRules["createProjects"]
	...
	err = rbac.VerifyBindingsForCrtb(acrc.client, rbac.LocalCluster, &crtbs[0], 1, 0)
	require.NoError(acrc.T(), err)
...
func (acrc *AggregatedClusterRolesCrtbTestSuite) TestCrtbInheritanceWithMultipleRules() {
	...
	log.Info("Verifying role bindings and cluster role bindings created for the user in the local cluster.")
	err = rbac.VerifyBindingsForCrtb(acrc.client, rbac.LocalCluster, &crtbs[0], 2, 0)
	require.NoError(acrc.T(), err)
...
func (acrc *AggregatedClusterRolesCrtbTestSuite) TestCrtbInheritanceWithTwoCrtbs() {
	...
	mainRules1 := rbac.PolicyRules["createProjects"]
	...
	log.Info("Verifying cluster role template bindings are created for the user")
	...
	err = rbac.VerifyBindingsForCrtb(acrc.client, rbac.LocalCluster, &crtbs[0], 1, 0)
	require.NoError(acrc.T(), err)
	err = rbac.VerifyBindingsForCrtb(acrc.client, rbac.LocalCluster, &crtbs[1], 1, 0)
	require.NoError(acrc.T(), err)
...
func (acrc *AggregatedClusterRolesCrtbTestSuite) TestCrtbNestedInheritance() {
	...
	childRules1 := rbac.PolicyRules["createProjects"]
...
func (acrc *AggregatedClusterRolesCrtbTestSuite) TestCrtbVerifyCrossClusterAccessRestriction() {
	...
	createdProject, _, createdUser, _, _, _, err := acrCreateTestResourcesForCrtb(acrc.client, acrc.cluster)
	require.NoError(acrc.T(), err)
	createdProject2, err := projects.CreateProjectUsingWrangler(acrc.client, rbac.LocalCluster)
	...
	mainRTName := createdMainRT.Name
	log.Infof("Adding user %s to downstream cluster %s with role %s", createdUser.Username, acrc.cluster.ID, mainRTName)
	_, err = rbac.CreateClusterRoleTemplateBinding(acrc.client, acrc.cluster.ID, createdUser, mainRTName)
	require.NoError(acrc.T(), err, "Failed to assign role to user")
	crtbs, err := rbac.VerifyClusterRoleTemplateBindingForUser(acrc.client, createdUser.ID, 1)
	require.NoError(acrc.T(), err, "CRTB not found for user")

	log.Infof("Adding user %s to local cluster with role %s", createdUser.Username, rbac.ManageNodes.String())
	_, err = rbac.CreateClusterRoleTemplateBinding(acrc.client, rbac.LocalCluster, createdUser, rbac.ManageNodes.String())
	require.NoError(acrc.T(), err, "Failed to assign role to user")
	...
	require.NoError(acrc.T(), rbac.VerifyUserPermission(acrc.client, rbac.LocalCluster, createdUser, "get", "projects", "", createdProject2.Name, false, true))
	require.NoError(acrc.T(), rbac.VerifyUserPermission(acrc.client, rbac.LocalCluster, createdUser, "list", "projects", "", "", false, true))
	require.NoError(acrc.T(), rbac.VerifyUserPermission(acrc.client, rbac.LocalCluster, createdUser, "create", "projects", "", "", false, true))
}
