...
func (crtbs *CRTBStatusFieldTestSuite) TestCreateCRTBAndVerifyStatusField() {
	...
	userCRTBList, err := GetClusterRoleTemplateBindingsForUser(crtbs.client, userID)
	require.NoError(crtbs.T(), err)
...
func (crtbs *CRTBStatusFieldTestSuite) TestCRTBStatusFieldReconciliation() {
	...
	require.NoError(crtbs.T(), err)
	userID := user.Resource.ID
	clusterID := crtbs.cluster.ID
	clusterName := crtbs.cluster.Name
	log.Infof("Debug test clusterID: %s AND clusterName: %s ", clusterID, clusterName)
	userCRTBList, err := GetClusterRoleTemplateBindingsForUser(crtbs.client, userID)
	require.NoError(crtbs.T(), err)
	...
	err = wait.PollUntilContextTimeout(context.TODO(), defaults.TwoMinuteTimeout, defaults.TwoMinuteTimeout, false, func(ctx context.Context) (bool, error) {
		updatedCRTB, err = GetClusterRoleTemplateBindingByName(crtbs.client, clusterID, initialCRTB.Name)
		if err != nil {
			return false, err
		}
...
func (crtbs *CRTBStatusFieldTestSuite) TestUpdateCRTBAndVerifyStatusField() {
	...
	err = wait.PollUntilContextTimeout(context.Background(), defaults.OneMinuteTimeout, defaults.OneMinuteTimeout, false, func(ctx context.Context) (bool, error) {
		updatedCRTBList, err := GetClusterRoleTemplateBindingsForUser(crtbs.client, userID)
		require.NoError(crtbs.T(), err)
		updatedUserCRTB := updatedCRTBList
