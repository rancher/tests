...
import (
	...
	"github.com/rancher/tests/actions/ingresses"
...
func (i *IngressRBACTestSuite) TestCreateIngress() {
	...
	for _, tt := range tests {
		i.Run(fmt.Sprintf("Validate ingress creation for %s role", tt.role), func() {
			...
			ingressPath := extensionsingress.NewIngressPathTemplate(pathType, "/api", steveWorkload.Name, 80)
			...
			case rbac.ClusterOwner.String(), rbac.ProjectOwner.String(), rbac.ProjectMember.String():
				...
				ingress, createErr = extensionsingress.CreateIngress(steveClient, ingressName, ingressTemplate)
