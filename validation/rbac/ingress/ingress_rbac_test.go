...
func (i *IngressRBACTestSuite) TestCreateIngress() {
	...
	for _, tt := range tests {
		i.Run("Validate creating ingress as user with role "+tt.role.String(), func() {
			...
			workload, err := deployment.CreateDeployment(i.client, i.cluster.ID, namespace.Name, 1, "", "", false, false, false, true)
			assert.NoError(i.T(), err)
			...
			ingressPath := extensionsingress.NewIngressPathTemplate(pathType, "/api", workload.Name, 80)
			ingressTemplate := extensionsingress.NewIngressTemplate(ingressNameForDeployment, namespace.Name, hostName, []networkingv1.HTTPIngressPath{ingressPath})
...
func (i *IngressRBACTestSuite) TestListIngress() {
	...
	for _, tt := range tests {
		i.Run("Validate listing ingress as user with role "+tt.role.String(), func() {
			...
			log.Info("As a admin, create a deployment")
			_, err = deployment.CreateDeployment(i.client, i.cluster.ID, namespace.Name, 1, "", "", false, false, false, true)
			assert.NoError(i.T(), err)

			ingressTemplateForDeployment, err := ingresses.CreateServiceAndIngressTemplateForDeployment(i.client, i.cluster.ID, namespace.Name)
			assert.NoError(i.T(), err)
	...
	for _, tt := range tests {
		i.Run("Validate deleting ingress as user with role "+tt.role.String(), func() {
			...
			case rbac.ClusterMember.String(), rbac.ReadOnly.String():
				...
				assert.NoError(i.T(), listErr)
				assert.Equal(i.T(), 1, len(ingressList.Items), "ingress should still exist")
			}
		})
	}
}
