...
| Step Number | Action | Data | Expected Result |
|------|--------|--------|-----------------|
| 1 | Configure same S3 backup target on both source and target clusters | | Both clusters use shared backup storage |
| 2 | Create volume with test data in source cluster | | Volume created with identifiable test data |
| 3 | Set up recurring backup schedule (every 30 minutes) | | Recurring backup configured |
| 4 | Wait for initial backup completion | | Backup appears in source cluster |
| 5 | Access target cluster Longhorn UI → Backup → Volume | | Backup from source cluster visible |
| 6 | Select backup and click "Create Disaster Recovery Volume" | | DR volume creation dialog opens |
| 7 | Configure DR volume with name "dr-test-volume" | | DR volume configuration set |
| 8 | Create DR volume and verify "Standby" state | | DR volume shows as standby |
| 9 | Monitor incremental synchronization progress | | DR volume stays synchronized |
| 10 | Simulate source cluster failure (power off nodes) | | Source cluster becomes unavailable |
| 11 | Activate DR volume in target cluster | | DR volume activation succeeds |
