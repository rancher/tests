//go:build (validation || windows || infra.rke2k3s || cluster.any || stress) && !infra.any && !infra.aks && !infra.eks && !infra.gke && !sanity && !extended
...
func TestCertRotationWindows(t *testing.T) {
	t.Parallel()

	c := certTest.Setup(t, defaults.RKE2)

	if c.ClusterConfig.Provider != providers.Vsphere {
