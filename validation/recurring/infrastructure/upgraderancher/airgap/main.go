...
func main() {
	...
	// Preserve the airgap tunnel host before upgrade; UpdateRancherConfigMap inside UpgradeAirgapRancher
	// overwrites rancher.host with the actual Rancher hostname, but downstream tests must connect
	// through 127.0.0.1:8443 (the SSH tunnel endpoint) since the server is not directly reachable.
	var originalHost any
	if rancherMap, ok := cattleConfig["rancher"].(map[string]any); ok {
		originalHost = rancherMap["host"]
	}

	client, cattleConfig, _, _ = upgradeairgap.UpgradeAirgapRancher(t, client, bastion, registry, testSession, cattleConfig, tunnel)

	// Restore the airgap tunnel host after the upgrade overwrites it.
	cattleConfig, err = operations.ReplaceValue([]string{"rancher", "host"}, originalHost, cattleConfig)
	if err != nil {
