...
var (
	qaseToken               = os.Getenv(qase.QaseTokenEnvVar)
	runIDEnvVar             = os.Getenv(qase.TestRunEnvVar)
	projectIDEnvVar         = os.Getenv(qase.ProjectIDEnvVar)
	testPackagePaths        = os.Getenv(qase.TestPackagePaths)
...
func updateTestInRun(client *upstream.APIClient, testCase testresult.GoTestResult, qaseTestCase upstream.TestCase, params []upstream.Params, testRunID int64) error {
	...
	logrus.Info(qaseTestCase.Params)
