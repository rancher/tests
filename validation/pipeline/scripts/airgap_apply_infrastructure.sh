...
if ! tofu -chdir=/root/go/src/github.com/rancher/qa-infra-automation/tofu/aws/modules/airgap apply -auto-approve -input=false tfplan 2>&1; then
    echo 'ERROR: Terraform apply failed'

    # Check if the failure was due to a stale plan by attempting to apply without a plan
    echo 'Checking if failure was due to stale plan and attempting recovery...'
    if tofu -chdir=/root/go/src/github.com/rancher/qa-infra-automation/tofu/aws/modules/airgap apply -auto-approve -input=false 2>&1; then
        echo 'SUCCESS: Terraform apply completed successfully without plan file (stale plan recovery)'
    else
        echo 'ERROR: Terraform apply failed even without plan file'
        exit 1
    fi
else
    echo 'SUCCESS: Terraform apply completed with plan file'
fi
...
# The file is already at /root/infrastructure-outputs.json which is in the shared volume
echo 'Infrastructure outputs generated at shared volume location: /root/infrastructure-outputs.json'

echo 'DEBUG: Checking for inventory file generation...'
echo 'DEBUG: Looking for inventory at: /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml'

# Check if the directory exists
if [ -d /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/ ]; then
    echo 'DEBUG: Inventory directory exists, listing contents:'
    ls -la /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/
else
    echo 'DEBUG: Inventory directory does not exist'
fi
...
if [ -f /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml ] && [ -s /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml ]; then
    ...
    echo 'Inventory file copied to shared volume location: /root/ansible-inventory.yml'

    # Copy to Ansible expected location (for Ansible playbook execution)
    mkdir -p /root/ansible/rke2/airgap/inventory/
    cp /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml /root/ansible/rke2/airgap/inventory.yml
    echo 'Inventory file copied to Ansible expected location: /root/ansible/rke2/airgap/inventory.yml'
    ...
    echo 'DEBUG: Detailed inventory file check:'
    echo 'File exists test:' [ -f /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml ] && echo "YES" || echo "NO"
    echo 'File not empty test:' [ -s /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/inventory.yml ] && echo "YES" || echo "NO"
    echo 'Checking for inventory file at source location:'
    ls -la /root/go/src/github.com/rancher/qa-infra-automation/ansible/rke2/airgap/inventory/ 2>/dev/null || echo 'Source directory does not exist'

    # Check if terraform created any inventory files in other locations
    echo 'DEBUG: Searching for any inventory files in the terraform directory...'
    find /root/go/src/github.com/rancher/qa-infra-automation -name "*inventory*" -type f 2>/dev/null || echo 'No inventory files found'

    # Since inventory file should be generated by Terraform, no fallback needed
    echo 'DEBUG: Inventory file generation is handled by Terraform module'
    echo 'DEBUG: If inventory file is missing, check Terraform configuration and state'
fi
