...
# This script clones and runs the Rancher Deployment playbook from qa-infra-automation

echo "=== Ansible Rancher Deployment Started ==="
...
# Check for the main Rancher Deployment playbook
RANCHER_DEPLOYMENT_PLAYBOOK="$QA_INFRA_PLAYBOOKS_DIR/deploy/rancher-helm-deploy-playbook.yml"
if [[ ! -f "$RANCHER_DEPLOYMENT_PLAYBOOK" ]]; then
    echo "ERROR: Rancher Deployment playbook not found at $RANCHER_DEPLOYMENT_PLAYBOOK"
...
echo "Using Rancher Deployment playbook from qa-infra-automation repository"
echo "Playbook path: $RANCHER_DEPLOYMENT_PLAYBOOK"
...
if [[ $ANSIBLE_EXIT_CODE -ne 0 ]]; then
    echo "ERROR: Rancher Deployment playbook failed with exit code $ANSIBLE_EXIT_CODE"
fi

echo "Rancher Deployment playbook execution completed"

# Copy playbook execution logs to shared volume
if [[ -f "ansible-playbook.log" ]]; then
    cp ansible-playbook.log /root/rancher-helm-deploy_execution.log
fi
