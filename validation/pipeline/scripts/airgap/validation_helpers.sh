...
validate_ssh_key_permissions() {
    ...
    local permissions=$(stat -c '%a' "$key_path" 2>/dev/null || stat -f '%A' "$key_path" 2>/dev/null)
    ...
    local dir_permissions=$(stat -c '%a' "$dir_path" 2>/dev/null || stat -f '%A' "$dir_path" 2>/dev/null)
