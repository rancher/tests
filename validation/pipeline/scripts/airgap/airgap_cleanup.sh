...
gather_system_state() {
    ...
    ps aux | grep -E "(tofu|terraform|ansible|kubectl)" > "$SHARED_VOLUME_PATH/pre-cleanup-processes.txt" 2>/dev/null || true
...
handle_timeout_cleanup() {
    ...
    if ps aux | grep -E "(ansible-playbook|tofu)" | grep -v grep; then
...
handle_manual_cleanup() {
    ...
    local backup_dir="$SHARED_VOLUME_PATH/manual-cleanup-backup-$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$backup_dir"
