...
func (a *ActiveDirectoryAuthProviderSuite) TearDownSuite() {
	if a.client != nil {
		adConfig, err := a.client.Management.AuthConfig.ByID(activeDirectory)
...
func (a *ActiveDirectoryAuthProviderSuite) TestEnableActiveDirectory() {
	...
	require.Equal(a.T(), authProvCleanupAnnotationValUnlocked, adConfig.Annotations[authProvCleanupAnnotationKey], "Annotation should be unlocked")
	...
	require.Equal(a.T(), client.Auth.ActiveDirectory.Config.ServiceAccount.Password, string(passwordSecret.Data["serviceaccountpassword"]), "Password mismatch")
}

func (a *ActiveDirectoryAuthProviderSuite) TestDisableActiveDirectory() {
	subSession := a.session.NewSession()
	defer subSession.Cleanup()
	...
	adConfig, err := waitForAuthProviderAnnotationUpdate(client, authProvCleanupAnnotationValLocked)
	...
	require.Equal(a.T(), authProvCleanupAnnotationValLocked, adConfig.Annotations[authProvCleanupAnnotationKey], "Annotation should be locked")
...
func (a *ActiveDirectoryAuthProviderSuite) TestAllowAnyUserAccessMode() {
	...
	err = verifyUserLogins(authAdmin, auth.ActiveDirectoryAuth, allUsers, "unrestricted access mode", true)
...
func (a *ActiveDirectoryAuthProviderSuite) TestGroupMembershipDoubleNestedGroupClusterAccess() {
	...
	for _, userInfo := range a.authConfig.DoubleNestedUsers {
		...
		userClient, err := loginAsAuthUser(authAdmin, auth.ActiveDirectoryAuth, user)
...
func (a *ActiveDirectoryAuthProviderSuite) TestAllowClusterAndProjectMembersAccessMode() {
	...
	newAuthConfig, err := updateAccessMode(a.client, AccessModeRestricted, allowedPrincipalIDs)
	...
	err = verifyUserLogins(authAdmin, auth.ActiveDirectoryAuth, allowedUsers, "restricted access mode", true)
	require.NoError(a.T(), err, "Cluster/project members should be able to login")

	err = verifyUserLogins(authAdmin, auth.ActiveDirectoryAuth, a.authConfig.Users, "restricted access mode", false)
...
func (a *ActiveDirectoryAuthProviderSuite) TestRestrictedAccessModeAuthorizedUsersCanLogin() {
	...
	err = verifyUserLogins(authAdmin, auth.ActiveDirectoryAuth, a.authConfig.Users, "required access mode", true)
...
func (a *ActiveDirectoryAuthProviderSuite) TestRestrictedAccessModeUnauthorizedUsersCannotLogin() {
	...
	err = verifyUserLogins(authAdmin, auth.ActiveDirectoryAuth, unauthorizedUsers, "required access mode", false)
