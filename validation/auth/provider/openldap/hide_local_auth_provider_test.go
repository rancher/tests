...
func (s *HideLocalAuthTestSuite) SetupSuite() {
	...
	disabled, err := extfeaturesapi.IsFeatureEnabled(s.client, hideLocalAuthFeature)
	require.NoError(s.T(), err)
...
func (s *HideLocalAuthTestSuite) TestHideLocalAuthEnabledWithOpenLDAP() {
	...
	err = extfeaturesapi.EnableFeatureFlag(s.client, hideLocalAuthFeature)
	require.NoError(s.T(), err)
...
func (s *HideLocalAuthTestSuite) TestHideLocalAuthEnabledWithNoExternalAuthEnabled() {
	...
	err = extuserapi.DeleteUser(s.client, testuser2.Username, true)
	require.NoError(s.T(), err)
...
func (s *HideLocalAuthTestSuite) TestUserWithManageUsersCanEditDeleteExistingLocalUserWhileHideLocalAuthProviderIsEnabled() {
	...
	_, err = extrbacapi.CreateGlobalRoleBinding(authAdmin, manageUsersBinding)
	require.NoError(s.T(), err, "Failed to create manage-users global role binding")
