...
func (k *KeycloakSAMLAuthProviderSuite) SetupSuite() {
	...
	k.adminPrincipalID = keycloakSAMLFixture.AdminPrincipalID
	k.entityID = keycloakSAMLFixture.EntityID
}
...
func (k *KeycloakSAMLAuthProviderSuite) TestKeycloakSAMLEnableRequiresExplicitAccessMode() {
	...
	require.Contains(k.T(), err.Error(), authactions.NotNullableError, "Enable should be rejected because the required field accessMode was omitted")
...
func (k *KeycloakSAMLAuthProviderSuite) TestKeycloakSAMLEnableIntoRestrictedAccessModeIsNotGuarded() {
	...
	logrus.Info("Enabling Keycloak SAML directly into restricted access mode with an empty allow list")
	err = k.client.Auth.KeycloakSAML.Enable()
	require.NoError(k.T(), err, "Enabling a SAML provider writes the auth config, and no login happens on that path for Rancher to check access against, so nothing stops the provider coming up restricted to an empty allow list")
...
func (k *KeycloakSAMLAuthProviderSuite) TestKeycloakSAMLNestedGroupsInAssertion() {
	...
	for _, tier := range tiers {
		...
		assertionGroups, err := authactions.KeycloakSAMLAssertionGroups(k.client, tier.user)
		require.NoError(k.T(), err, "Failed to capture the assertion Keycloak issues for user [%v]", tier.user.Username)

		require.Contains(k.T(), assertionGroups, tier.group, "Keycloak should assert the group [%v] user [%v] belongs to directly, otherwise no binding of any kind can reach them", tier.group, tier.user.Username)

		for _, ancestor := range tier.ancestors {
			require.NotContains(k.T(), assertionGroups, ancestor, "Keycloak asserts direct memberships alone, so ancestor group [%v] must not appear for user [%v]; Rancher's SAML provider resolves no hierarchy of its own and believes this list verbatim, so anything absent here is invisible to Rancher", ancestor, tier.user.Username)
		}
	}
}
...
func (k *KeycloakSAMLAuthProviderSuite) TestKeycloakSAMLParentGroupBindingDoesNotReachNestedMembers() {
	...
	for _, userInfo := range nestedMembers {
		...
		_, err = userClient.Steve.SteveType(clusters.ProvisioningSteveResourceType).List(nil)
		require.Error(k.T(), err, "User [%v] sits beneath group [%v] rather than in it, and Keycloak asserts only their direct group, so the binding on [%v] must not reach them", user.Username, k.authConfig.Group, groupPrincipalID)
		require.Contains(k.T(), err.Error(), "Resource type [provisioning.cattle.io.cluster] has no method GET", "Should indicate insufficient permissions")
	}
...
func (k *KeycloakSAMLAuthProviderSuite) TestKeycloakSAMLFullGroupPathBinding() {
	...
	for _, tier := range pathTiers {
		...
		require.Contains(k.T(), assertionGroups, tier.path, "Keycloak should now assert group [%v] as the path [%v], which is what makes a nested group distinguishable from any other group sharing its name", tier.bareName, tier.path)
