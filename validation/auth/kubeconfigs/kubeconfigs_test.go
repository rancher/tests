...
func (kc *KubeconfigTestSuite) TestDeleteKubeconfigAsAdmin() {
	...
	log.Infof("Admin deleting all kubeconfigs")
	...
	log.Infof("Verifying all kubeconfigs are deleted")
	allKcs, err := kubeconfigapi.ListKubeconfigs(kc.client)
	require.NoError(kc.T(), err)
	for _, kubecfg := range allKcs.Items {
		require.NotContains(kc.T(), []string{adminKc.Name, firstUserKc.Name, secondUserKc.Name}, kubecfg.Name)
	}
}
...
func (kc *KubeconfigTestSuite) TestDeleteKubeconfigAsNonAdmin() {
	...
	require.NoError(kc.T(), err)
	_, err = kubeconfigapi.GetKubeconfig(firstUserClient, firstUserKc.Name)
	require.Error(kc.T(), err)
	require.True(kc.T(), k8serrors.IsNotFound(err))

	err = kubeconfigapi.DeleteKubeconfig(secondUserClient, secondUserKc.Name)
	require.NoError(kc.T(), err)
	_, err = kubeconfigapi.GetKubeconfig(secondUserClient, secondUserKc.Name)
	require.Error(kc.T(), err)
	require.True(kc.T(), k8serrors.IsNotFound(err))
}
...
func (kc *KubeconfigTestSuite) TestKubeconfigAfterBackingTokensDeleted() {
	...
	_, err = kubeconfigapi.GetKubeconfig(kc.client, adminKc.Name)
	require.Error(kc.T(), err)
	require.True(kc.T(), k8serrors.IsNotFound(err))
}
