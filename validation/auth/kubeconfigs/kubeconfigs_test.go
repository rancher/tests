...
func (kc *KubeconfigTestSuite) TestCreateKubeconfigAsAdmin() {
	...
	err = kubeconfigapi.VerifyAllContextsUsable(kubeconfigapi.KubeconfigFile)
	require.NoError(kc.T(), err)
}
...
func (kc *KubeconfigTestSuite) TestCreateKubeconfigForUnauthorizedUser() {
	...
	require.True(kc.T(), errors.IsForbidden(err))
}

func (kc *KubeconfigTestSuite) TestKubeconfigAccessForMultipleUsers() {
	subSession := kc.session.NewSession()
	defer subSession.Cleanup()

	log.Infof("Creating a user and adding the user to the downstream cluster with role %s", rbac.ClusterOwner.String())
	...
	log.Infof("Creating another user and adding the user to the same downstream cluster with role %s", rbac.ClusterOwner.String())
	...
	log.Infof("As first user %s with role %s, creating a kubeconfig for cluster: %s", firstUser.Name, rbac.ClusterOwner.String(), kc.cluster.ID)
	...
	log.Infof("As second user %s with role %s, creating a kubeconfig for cluster: %s", secondUser.Name, rbac.ClusterOwner.String(), kc.cluster.ID)
	...
	kcObjForbidden, err := kubeconfigapi.GetKubeconfig(firstUserClient, secondUserKubeconfig.Name)
	require.Error(kc.T(), err, "First user should not be able to access second user's kubeconfig")
	require.Nil(kc.T(), kcObjForbidden, "No kubeconfig should be returned to unauthorized user")
	require.True(kc.T(), errors.IsNotFound(err))
