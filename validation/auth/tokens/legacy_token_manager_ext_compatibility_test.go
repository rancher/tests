...
func (t *LegacyTokenManagerExtTokenCompatibilityTest) SetupSuite() {
	...
	defaultTTLString, err := extsettingsapi.GetGlobalSettingDefaultValue(t.client, extsettingsapi.AuthTokenMaxTTLMinutes)
	require.NoError(t.T(), err)
...
func (t *LegacyTokenManagerExtTokenCompatibilityTest) TestAuthenticateWithLegacyTokenManagerUsingExtToken() {
	...
	require.NoError(t.T(), err)
	standardUserExtToken, err := tokenapi.CreateExtToken(standardUserClient, t.defaultExtTokenTTL)
	require.NoError(t.T(), err)
	...
	err = tokenapi.AuthenticateWithExtToken(baseURL, standardUserExtToken.Name, standardUserTokenValue, legacyTokenAPIPath)
	require.NoError(t.T(), err)
}
...
func (t *LegacyTokenManagerExtTokenCompatibilityTest) TestDeleteLegacyTokenUsingExtToken() {
	...
	err = tokenapi.DeleteLegacyTokenWithExtToken(standardUserClient, standardUserLegacyToken.ID, standardUserExtToken.Status.BearerToken)
	require.NoError(t.T(), err)
}
