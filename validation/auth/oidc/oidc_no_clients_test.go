...
func (s *OIDCNoClientsTestSuite) TestPublicEndpointsOmitCORSHeaderWithNoOIDCClients() {
	s.requireNoOIDCClients()

	logrus.Info("Verifying the public OIDC endpoints set the security headers and omit Access-Control-Allow-Origin with no OIDCClients configured")
...
func (s *OIDCNoClientsTestSuite) TestUserInfoRejectsRevokedTokenAfterLastOIDCClientDeleted() {
	...
	logrus.Info("Verifying /oidc/userinfo returns 401 for the revoked access token")
