...
func (s *OIDCTestSuite) SetupSuite() {
	...
	require.NoError(s.T(), features.EnableFeatureFlag(client, oidcauth.OIDCProviderFeatureFlag),
		"Failed to enable oidc-provider feature flag")
	require.NoError(s.T(), features.WaitForRancherReady(client),
...
// craftJWTWithIntKid returns a JWT-shaped string whose header `kid` field is an integer rather than a string.
// Used by TestSecurityNonStringKidDoesNotPanic to verify Rancher's resilience to malformed JWT headers.
func craftJWTWithIntKid() (string, error) {
	header := map[string]interface{}{"alg": "RS256", "typ": "JWT", "kid": 12345}
	hBytes, err := json.Marshal(header)
	if err != nil {
		return "", err
	}
	payload := map[string]interface{}{"sub": "test-user", "iss": "https://test.example.com", "exp": 9999999999}
	pBytes, err := json.Marshal(payload)
	if err != nil {
		return "", err
	}
	return base64.RawURLEncoding.EncodeToString(hBytes) + "." +
		base64.RawURLEncoding.EncodeToString(pBytes) + ".fakesignature", nil
}
...
func (s *OIDCTestSuite) TestOIDCClientRegisteredInNormanAPI() {
	subSession := s.session.NewSession()
	defer subSession.Cleanup()

	logrus.Info("Verifying OIDCClient schema is registered in Norman API")

	resp, err := s.oidcAPI.RawRequest("GET", "/v3/schemas/oidcclient",
		"Bearer "+s.client.RancherConfig.AdminToken)
	require.NoError(s.T(), err)
	require.Equal(s.T(), http.StatusOK, resp.StatusCode,
		"OIDCClient schema must be registered in Norman: status=%d body=%s", resp.StatusCode, resp.Body)
}
...
func (s *OIDCTestSuite) TestOIDCClientCreatableViaNormanAPI() {
	...
	logrus.Info("Verifying POST /v3/oidcclients creates an OIDCClient via Norman API")

	normanClientName := namegen.AppendRandomString("norman-oidc")

	rancherURL := strings.TrimRight(s.client.RancherConfig.Host, "/")
	if !strings.HasPrefix(rancherURL, "https://") && !strings.HasPrefix(rancherURL, "http://") {
	...
	resp, err := clientbase.Do(httpClient, "POST", rancherURL+"/v3/oidcclients", body, map[string]string{
		"Authorization": "Bearer " + s.client.RancherConfig.AdminToken,
	...
	require.Equal(s.T(), http.StatusCreated, resp.StatusCode,
		"POST /v3/oidcclients must return 201 Created when Norman is wired: status=%d body=%s", resp.StatusCode, resp.Body)
}
...
func (s *OIDCTestSuite) TestSecurityNonStringKidDoesNotPanic() {
	...
	craftedJWT, err := craftJWTWithIntKid()
	require.NoError(s.T(), err)
...
func (s *OIDCTestSuite) TestTokenWhenFeatureFlagDisabled() {
	...
	err := features.DisableFeatureFlag(s.client, oidcauth.OIDCProviderFeatureFlag)
	...
	err = features.EnableFeatureFlag(s.client, oidcauth.OIDCProviderFeatureFlag)
	require.NoError(s.T(), err, "Should be able to re-enable oidc-provider feature flag")
	err = features.WaitForRancherReady(s.client)
